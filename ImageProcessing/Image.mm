 ...
 * Input:
 *      black: edge
 *      other: candidate of region boundary
 * Output:
 *      black: detected region
 *      white: the rest
 */
+ (ImageWrapper *) regionDetectedImageWithUIImage:(UIImage *) image x:(int) x y:(int) y
{
	...
	ImageWrapper *greyScale = Image::createImage(image, imageWidth, imageHeight);
    //ImageWrapper *mono = greyScale.image->mono();
    
    NSLog(@"Detecting connected region in image of size:%dx%d -> %dx%d", (int) imageWidth, (int) imageHeight, x, y);
    
    return greyScale.image->connectedRegion(x, y, NULL);
}

- (ImageWrapper *) regionDetectedImageAtX:(int) x y:(int) y
{
    return self.image->connectedRegion(x, y, NULL);
}
...
void Image::extractConnectedRegion(int x, int y, std::vector<ImagePoint> *points) {
	...
	left=x-1;
	right=x+1;
	while(left>=0 && (*this)[y][left] != sample) {
		(*this)[y][left] = sample;
		(*points).push_back(ImagePoint(left,y));		
		left--;
	}
	while(right<m_width && (*this)[y][right] != sample) {
		(*this)[y][right] = sample;
	...
	for(int i=left; i<=right; i++) {
		if(i>=0 && i<m_width) {
			if(y>0 && (*this)[y-1][i] != sample) {
				extractConnectedRegion(i, y-1, points);
			}
			if(y<(m_height-1) && (*this)[y+1][i] != sample) {
				extractConnectedRegion(i, y+1, points);
			}
		}
	}
 ...
 *
 *  sample[y][x]:
 *      black: already marked as connected region. returns immediately
 *      other: candidate of the connected region.
 */
void Image::extractConnectedRegion2(int x, int y, std::vector<ImagePoint> *points) {
    uint8_t sample = (*this)[y][x];
    if ( sample == 0) return;
	(*points).push_back(ImagePoint(x,y));
    (*this)[y][x] = 0;  // mark detected
    
    //NSLog(@"(%2dx%2d): %d", x, y, sample);
	...
	for(int i=left; i<=right; i++) {
		if(i>=0 && i<m_width) {
			if(y>0 && (*this)[y-1][i] == sample) {
				extractConnectedRegion2(i, y-1, points);
			}
			if(y<(m_height-1) && (*this)[y+1][i] == sample) {
				extractConnectedRegion2(i, y+1, points);
			}
...
ImageWrapper *Image::connectedRegion(int x, int y, std::vector<ImagePoint> *ppoints)
{
    
    std::vector<ImagePoint> points;

    this->extractConnectedRegion2(x, y, &points);
    ...
    uint8_t region = 255;
    memset(result->m_imageData, background, m_width * m_height);
    for(int i = 0; i < points.size(); i++) {
        ImagePoint &point = points[i];
        (*result)[point.y][point.x] = region;
    }

    if ( ppoints ) {
        std::copy( points.begin(), points.end(), ppoints->begin());
    }
	return [ImageWrapper imageWithCPPImage:result];
}
...
{
    std::sort( points.begin(), points.end(), sortByY1);
    std::sort( points.begin(), points.end(), sortByX1);
}
